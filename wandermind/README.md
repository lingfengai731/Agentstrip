...
Unlocking the current professional route includes three parameter adjustments for that same trip. These adjustments are separate from AI credits and do not represent human deep customization or three new travel orders. Exhausting AI adjustments never silently converts an AI request into a professional route. The legacy `ai-tool.html?professional=1` query remains a compatibility redirect to the Bali professional planner.
...
- 🌍 **任意目的地动态面板**，登录后输入城市，由 AI 生成待核验的时区 / 推荐区域 / 旅行贴士草稿，天气独立更新
...
| 目的地面板 | 独立实时天气、货币提示、当季信息、推荐区域、旅行贴士 |
| 任意目的地 | 登录后输入任何城市 → AI 生成待核验的面板草稿；天气由独立服务更新 |
| 💰 比价 Tab | 右栏独立 tab，子页签切换酒店/机票 |
| 🏨 酒店比价 + 区域筛选 | 预设 4 大目的地提供住宿区 chips；自定义目的地 AI 草稿生成 3 个住宿区 |
...
# 🫘 AI 用量限额 + 旅行豆（防滥用）
# 每个已登录账号默认 5 次免费 AI 问答，用完需充值豆。
# 未登录用户可浏览公开路线，但必须登录后调用 AI，避免匿名轮换身份消耗模型额度。
FREE_USE_LIMIT=5
...
| POST | `/api/dest_info` | 巴厘岛/京都/巴黎/圣托里尼五语精选资料静态直出；任意目的地或 `enhance=true` 需登录并生成 AI 草稿。响应以 `source_kind` 标识 `curated` / `ai_generated`，天气不在此接口中返回 |
