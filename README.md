...
| PayPal 自动支付 | 🟡 Sandbox 主路径已闭环、待外部异常流与 Live 门禁 | 站长已用同一 Sandbox Personal 买家完成 USD 1.49 付款，并在刷新、重新进入、行程调整及账户入口中确认权益持续恢复，成功支付阶段已闭环。本轮增加买家取消后的本地订单关闭与全新重试、退款复核队列可见性；87 项权限测试及 390/768/1440 浏览器取消态通过。拒绝、重复 webhook、退款已有本地状态机测试，但真实 Sandbox webhook/退款与 Live 资金仍是独立门禁，不能把 Sandbox 成功等同于真实收款 |
