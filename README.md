...
| PayPal 自动支付 | 🟡 生产代码已部署、待 Sandbox 凭据与交易验收 | Orders v2 创建/捕获、服务器金额复核、webhook 签名与幂等、退款转人工复核、权益写入及五语言响应式按钮已上线；本地 82 项权限测试通过。产品展示价保持 ¥9.9，PayPal 测试价为 USD 1.49。当前生产配置 API 明确返回 `enabled:false`，因此不会显示 PayPal；真实 Sandbox 买家支付、webhook 与退款验收仍未完成 |
