...
| 人工收款确认 | ✅ 已完成 | 微信/支付宝与可选银行卡转账共用待确认订单；真实银行资料只从服务器私密环境返回给已登录用户，管理员在 AI 工具账户中心确认到账并解锁 |
...
| 💰 收款 | 专业路线保留 PayPal Sandbox；中国用户可用微信/支付宝及服务器私密配置的银行卡转账并由管理员确认。司机、交通与活动由当地提供者另行报价并直接收款 |
