...
| 图片治理 | ✅ 首图生产闭环 | 发布 manifest 共 119 张：109 张记录用户统一授权，10 张精确地点图记录 Wikimedia Commons 的来源、作者、CC0 / CC BY / CC BY-SA、哈希、WebP、移动端缩略图与可见署名；`lava stone plant.jpg` 已按用户本轮授权关联巴杜尔黑色熔岩原；清单匹配会自动带入五语言资料 |
...
| 重要地点多图 | ✅ 已使用现有授权素材 | 百沙基寺 4 张、巴杜尔吉普 2 张、温泉 2 张；同一银饰插图可安全服务两个不同 POI，不再因全局去重使其中一个地点无图。其余地点不使用错误图片凑数 |
| 自动验证 | ✅ 本地通过 | 9 组小程序回归、329 条契约、64/64 POI 媒体覆盖、WebKit 390/1440 px 图片与布局检查及 `git diff --check` 通过 |

> `hiking-view1/2/3.jpg`、`batur-jeep-sunrise.jpg` 与 `batur-jeep.jpg` 已发现于原始素材目录，但尚未进入授权 manifest；在站长确认公开使用权前不会打包或发布。小程序 Upload、提审与公开发布仍是独立门禁。
