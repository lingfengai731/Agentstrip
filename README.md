...
- 首页顺序调整为作品集 → 美食 → 退出登录。保留原有品牌与布局。
- 图库和景点详情显式启用网络 WebP；慢图保留节点以接收迟到的成功回调；重试缩略图不再改为更大的原图，详情支持单张重试。
- 路线生成保持一次 POST，等待上限由15秒调为45秒；超时不伪造路线或权益，不自动重复提交。页面卸载、账号变更的迟到结果被丢弃。
- 图库与生成失败处新增“检查连接”：只探测公开 healthz 和示例图片，不发送账号、令牌或行程；结果可复制，区分 request/downloadFile 域名、TLS和超时。
- 项目及本机开发工具开启合法域名校验。微信后台需要核对 request 的 wandermind.cc 和 downloadFile 的 wandermind.cc/res.cloudinary.com；后台配置和手机实际网络仍待外部验收。
- 本机直连 healthz 15秒超时，系统代理返回200；不能把代理下的图片加载通过写成真机问题已解决。本批只改小程序，无网站后端部署必要。

## 2026-09-20 行程与作品集修复

- 网站、小程序明确区分“为当天选餐厅”和“加入用餐安排”；网站按钮居中并保留 44px 点击区域。
- 小程序先展示每日行程，佩妮达加游移到末尾并默认收起；地点改为整行入口。已保存的路线可经确认后恢复推荐安排，其他路线不受影响。
- 附近候选地点按当天实际地点所在片区筛选，不再把整个大区域的照片重复推荐给每一天；R1 情人崖仍在第 3 天。
- 作品集分批显示、增加图片加载反馈与单张重试；管理员新增作品暂不可用时不阻塞现有图库。
- 本机开发者工具采用 SYSTEM 代理，并只对 `servicewechat.com` 及子域绕过代理；取代曾导致网站图片直连超时的 DIRECT 设置。备份位于本机交付目录，不能把此修复当作手机网络配置已验证。
- 本轮验收范围为路线—景点照片—作品集—用餐安排，不代表网站所有功能均已全面验收。Preview 不等于 Upload、提审或公开发布。
...
| 人工收款确认 | ✅ 已完成 | 仅微信/支付宝二维码共用待确认订单；不采集、不保存、不展示个人银行卡号，管理员在 AI 工具账户中心确认到账并解锁 |
| 邀请积分 | ✅ 已完成 | 邀请者 10 分、受邀者 5 分，24 小时后生效；30 分兑换并实际生成 1 次专业路线，每月最多计 5 位有效邀请 |
| 巴厘岛路线体系 | ✅ 第一版 | G1–G7 地理事实层 + R1–R6 主题路线家族；65 个 POI 中 60 个稳定事实已核验、2 个待确认规范身份、3 个受供应商门禁；移动端可横向浏览，路线先于作品集 |
...
| 图片治理 | ✅ 授权素材发布闭环 | 发布 manifest 共 125 张：115 张记录用户统一授权，10 张精确地点图记录 Wikimedia Commons 的来源、作者、CC0 / CC BY / CC BY-SA、哈希、WebP、移动端缩略图与可见署名；5 张巴杜尔徒步/吉普照片与 1 张罗威纳海豚照片已按 2026-09-11 明确授权关联对应地点；清单匹配会自动带入五语言资料 |
| 巴厘岛视觉数据库 | ✅ POI 全覆盖 | Portfolio 保持 D8 分类与来源门禁；路线选择器的 65 个非退役 POI 已实现 65/65 视觉覆盖，体验、区域或地形参考均明确标注，不用错误图片凑数 |
...
| 项目 | 状态 | 完成效果 |
|------|------|----------|
| 路线地点视觉 | ✅ 本地完成 | 65/65 个非退役 POI 均有路线选择器视觉与介绍；体验、区域或地形参考均按范围标注，不再显示“该地点照片尚未接入” |
| 全站搜索 | ✅ 本地完成 | 7 个公开页面的放大镜进入五语言搜索页，可检索站点页面、R1–R6 和 65 个 POI；支持 GET 分享、键盘与手机触控 |
| 一至两日体验套餐 | ✅ 本地完成 | 9 套可筛选产品覆盖 Batur、Ubud、Penida、南部悬崖、东部与罗威纳；Batur 吉普路线可衔接 Black Lava，选择后带入司机询价，不伪装成即时预订或固定成交价 |
...
### 2026-09-05 小程序作品集与支付合规边界（发布前）

| 项目 | 状态 | 完成效果 |
|------|------|----------|
| 小程序作品集 | ✅ Preview 编译通过 | 首页新增“岛屿上的真实瞬间”；复用网站已授权清单与公开 Portfolio，不把大图塞进小程序包。筛选、缩略图、长按保存、空态和重试均为手机交互设计 |
| 路线景点多图 | ✅ Preview 编译通过 | R1–R6 已出现的 37 个地点均可点开；详情页使用原生 `swiper` 展示该地点当前可用的全部图片、介绍、图片范围与版权署名。当前数据 37/37 有图，其中 4 个地点已有多张，其余地点先诚实展示一张，不用错误图片凑数 |
| 支付商品与司机服务隔离 | ✅ 代码与测试完成 | ¥9.9 明确定义为数字专业路线访问权和同一行程 3 次参数调整，不包含司机、交通、酒店或活动代订；供应商另行报价、直接收款 |
| PayPal Live 防误开 | ✅ 代码与测试完成 | Sandbox 保持可用；Live 除凭据外还必须显式设置 `PAYPAL_LIVE_APPROVED=1`。该门禁仅可在支付平台书面批准真实商品和资金流后开启，不能用虚假资质或商品分类绕过审核 |
| 中国大陆人工收款 | ✅ 微信/支付宝可用；银行卡代码完成、待站长私密配置 | 微信/支付宝继续由管理员核对到账；银行卡账户仅从 Render 私密环境读取并只向已登录付款用户展示。银联在线收银台在完成商户入网前保持不可用，不伪造支付入口 |
| 司机起步估价 | ✅ 网站已有；小程序本轮补齐 | Dicky/Gede 共用已确认的 IDR 700k/10 小时、IDR 500k/6 小时起步规则，超时 IDR 70k/小时；不再按人数加价。机场、佩妮达及特殊路线单列为未计入项，最终由司机回复确认并直接收款 |

> 小程序本轮仅完成代码、自动测试与 Preview 编译，尚未 Upload、提审或发布。网站/后端是否进入生产，以本分支合并后的精确 Render 部署核验为准。

### 2026-09-11 Upload 前新客户审计

| 项目 | 状态 | 完成效果 |
|------|------|----------|
| 全站交互巡检 | ✅ 本地通过 | 11 个页面 × 手机/平板/桌面共 33 组检查；无页面脚本错误、断锚点、重复 ID、缺失图片替代文本或真正无标签字段。公开按钮和链接回归覆盖 204 个按钮、419 个链接 |
| 小程序目的地边界 | ✅ Preview 编译通过 | 首发入口只公开巴厘岛；京都、巴黎、圣托里尼与自定义目的地仍保留为未来扩展数据，但在完成实地调研和当地服务承接前不向用户展示 |
| 路线与规划表达 | ✅ 本地通过 | 解锁入口保留当前路线、语言和来源；游客界面隐藏内部区域编号；“轻松/平衡/充实”改为每天 1–2、2–3、3–4 个地点，预算明确为对应币种 |
| 手机触控与可访问性 | ✅ 本地通过 | 路线展开、前后移动、移除地点、恢复推荐、导航、主题和 AI 主要操作补足触控尺寸、键盘焦点与可读名称 |
...
### 2026-09-11 巴厘岛首发聚焦与火山路线影像校准

| 项目 | 状态 | 完成效果 |
|------|------|----------|
| 小程序首发目的地 | ✅ 本地通过 | 首页、缓存恢复与深链统一收敛到巴厘岛；未来目的地数据未删除，公开入口暂不展示，避免把尚未实地核验的产品包装成可落地服务 |
| R5 火山路线 | ✅ 本地通过 | 第 1 天为百沙基寺与阿贡火山高地语境，第 2 天明确二选一的徒步或日出吉普，第 3 天为温泉恢复；三天不再重复同一张图 |
| Batur 图片语义 | ✅ 本地通过 | 温泉首图改为金塔马尼温泉住宿的区域参考，旧 Toyabungkah 温泉图降为次序靠后的区域参考；徒步、吉普和温泉不再冒充同一精确场地 |
| Jeep 运营方边界 | ✅ 本地通过 | 展示 Dicky 推荐的 CV Bali Jeep Batur（Pak Nanok）作为当地运营方线索，并明确套餐、接送、车辆、保险与最终价格仍需预订前确认 |
| 重要地点多图 | ✅ 已使用现有授权素材 | 百沙基寺 4 张、巴杜尔徒步至少 4 张、巴杜尔吉普至少 4 张、温泉 2 张、罗威纳观海豚至少 2 张；同一银饰插图可安全服务两个不同 POI，不再因全局去重使其中一个地点无图。其余地点不使用错误图片凑数 |
| 自动验证 | ✅ 本地通过 | 9 组小程序回归、329 条契约、64/64 POI 媒体覆盖、WebKit 390/1440 px 图片与布局检查及 `git diff --check` 通过 |
...
| 🏝️ 巴厘岛 Portfolio | R1–R6 公共路线 + 专业路线预览/解锁 + 65/65 POI 视觉预览 + 9 个一至两日体验套餐 + 找司机 CTA；动态作品按地点去重 | ✅ 生产已部署 |
| 🔎 全站搜索 | 7 个公开页统一入口；检索页面、R1–R6 和 65 个 POI，支持五语言、GET 分享、键盘与手机 | ✅ 生产已部署 |
...
| 5 · P1 | **PayPal 自动支付生产闭环** | 六项 Render Sandbox 变量与 webhook 已配置；Sandbox Personal 成功付款及同账号权益恢复已完成。商品已收敛为数字路线访问权，明确不含交通和代订；Live 新增显式书面批准门禁。外部验收仍需一次性 Sandbox 订单完成取消/拒绝、真实 webhook、退款与对账；PayPal 或其他收单方未书面批准真实商品与资金流前不得切 Live |
...
| 13 · P1 | **微信小程序 v1 与发布门禁** | v1 已接入邮箱/微信统一身份、验证码、AI、R1–R6、专业路线权益、历史、无邮箱司机中转和微信官方内容安全核验。站主已在真机 Preview 验收无白屏、品牌名、错误密码、邮箱登录和微信一键登录；本轮又迁移网站作品集和路线景点多图详情：37/37 公共路线地点均可点开，图片保留范围说明与署名，主包仍约 1.5 MB。新 Preview 待站主扫码复验；未上传、未提审、未发布 |
...
| 💰 收款 | 专业路线保留 PayPal Sandbox；中国用户可用微信/支付宝二维码并由管理员确认。网站不提供个人银行卡转账；司机、交通与活动由当地提供者另行报价并直接收款 |
