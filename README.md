...
| 图片治理 | ✅ 首图生产闭环 | 发布 manifest 共 113 张：108 张记录用户统一授权，5 张精确地点图记录 Wikimedia Commons 的来源、作者、CC0 / CC BY / CC BY-SA、哈希、WebP、移动端缩略图与可见署名；`Pura Tanah Lot.jpg` 已在生产完成对象存储直传、数据库草稿、预览、发布、公开读取与隐藏回滚；清单匹配会自动带入五语言资料 |
| 巴厘岛视觉数据库 | ✅ D8 当前版 | 当前 Portfolio 首屏精选 12 个实拍内容位，可展开全部 37 个静态内容位；统一为 Landscapes / Culture / Experiences 三类，地点信息拆为区域、地点类型、路线关联、标签、氛围、摄影风格和核验状态，并可结构化带入 AI 或司机询价 |
...
| 优先级 | 内容 | 说明 |
|--------|------|------|
| 0 · P0 | **发布候选生产门禁** | 当前 PR #3 分支已完成本地功能与视觉闭环，但公开生产仍是旧基线。验收顺序固定为：只读确认生产 PostgreSQL 与稳定强 `SECRET_KEY` 存在（不显示值）→ Render 安全 canary/部署 → 两个外部客户端的代理地址隔离烟测 → 生产五语言与 320/390/768/1440 全链路 E2E → PR ready/merge。任一步失败即保持 Draft，不把分支结果写成已上线 |
| 1 · P0 | **巴厘岛剩余 POI 来源复核** | R1 主干、R2、R3、R4 与 R6 的免费路线稳定事实已核验；分支全库现为 62 个 POI：57 个 `verified`、2 个 `pending_review`、3 个 `needs_supplier_confirmation`。Batur Natural Hot Spring 已收敛到 Bangli 政府目录中的明确场所；Mount Batur 仅保留区域级徒步节点并明确存在多个入口；Thousand Islands Viewpoint 与具体 Mount Batur 入口仍待唯一身份或入口证据。验收：公开路线引用的稳定事实有官方/政府或场所一手来源，营业时间、票价、课程、仪式、火山状态、天气、水况、船班与交通继续在出发前实时复核 |
...
| 4 · P1 | **批量 Portfolio 治理** | 首图生产生命周期已闭环；当前 33 张 D8 图片已补齐中/英/日/韩/印五语言标题、说明和替代文本。坎普罕山脊、德格拉朗梯田、乌布艺术市场、梅拉斯蒂海滩和乌布皇宫新增精确地点图，并为手机地点选择器提供独立缩略图；`bali-12.jpg` 已关联乌布圣猴森林，`bali-3.jpg` 已由标志性岬角影像与政府/国家旅游资料关联 Kelingking Beach Viewpoint；`Nyepi.jpg` 已纠正为塞米亚克村社神庙的社区文化聚会；`bali-1.jpg`、`bali-2.jpg` 与 `bali-4.jpg` 因缺少唯一地点证据保持未知地点且不开放 AI/司机交接。剩余 24 张仍按地点证据、路线关联和五语言资料分批审核。未确认素材不可发布，公开 API 与 UI 数量口径必须一致 |
| 5 · P1 | **在线自动支付**（Stripe / 微信商户） | 目前是收款码 + 管理员人工确认。验收：收款主体确定，支付回调、签名、幂等、对账、退款/失败与人工兜底通过测试后再上线 |
| 6 · P1 | **持久化反滥用** | ✅ 代码与本地测试已完成：司机询价改为 SQLite/Postgres 共享的 HMAC 伪匿名 30 分钟数据库计数，重启或多实例不再依赖进程内字典；数据库异常时 fail-closed，不保存姓名、邮箱、预算或行程正文。真实 Neon/Postgres 与 Render 代理地址识别仍须在部署前后分别烟测 |
