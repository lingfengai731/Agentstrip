...
| 图片治理 | 🟡 上线前门禁 | 108 张候选素材均已记录统一授权、哈希、Web 优化副本与发布 manifest；首张真实单图 E2E 候选为 `Pura Tanah Lot.jpg`，已补齐五语言元数据和重复 SHA 门禁，生产 Cloudinary 凭据及强管理员引导未配置前不上传 |
...
> Portfolio 生产单图 E2E 还需要在 Render 配置 `CLOUDINARY_CLOUD_NAME`、`CLOUDINARY_API_KEY`、`CLOUDINARY_API_SECRET`，并启用强管理员账号；缺少任一门禁时保持草稿/不上线。
...
| 🟡 内容管理 | **Portfolio Content Manager** | `/admin/portfolio` 第一阶段已实现多图直传、草稿、预览、发布、隐藏、归档、替换和排序，并保留 37 张静态图片作为故障回退；生产环境还需配置 Cloudinary 三项变量后才能实际上传 |
