...
| PayPal 自动支付 | 🟡 Sandbox 已启用、待真实测试交易验收 | Orders v2 创建/捕获、服务器金额复核、webhook 签名与幂等、退款转人工复核、权益写入及五语言响应式按钮已上线；本地 83 项权限测试通过。站长已配置六项 Render Sandbox 变量，公开配置接口验证为 `enabled:true`、USD 1.49 且不暴露 Secret；真实 Sandbox 买家支付、webhook 与退款验收仍未完成 |
...
| 5 · P1 | **PayPal 自动支付生产闭环** | 六项 Render Sandbox 变量与 webhook 已配置，公开配置验证为启用且未泄露 Secret；v2 `DECLINED/PENDING` 及当前 Checkout 状态兼容已补齐。验收：使用 Sandbox Personal 买家完成一次支付、重复回调、取消与退款测试；通过后再单独决定切换 Live，不把 Sandbox 成功或银行卡绑定等同于真实收款验收 |
