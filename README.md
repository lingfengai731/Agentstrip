...
| 优先级 | 内容 | 说明 |
|--------|------|------|
| 🟡 内容核验 | **巴厘岛 POI 逐条来源复核** | R1 主干 15 个地点已核验、火山 Jeep 等待供应商；R3 的 8 个主路线地点已全部用官方/政府来源核验稳定事实。全库 19 个 `verified`、30 个 `pending_review`、1 个 `needs_supplier_confirmation`；营业时间、票价、仪式、天气与交通继续要求出发前实时复核 |
| 🟡 内容管理 | **批量 Portfolio 治理** | `/admin/portfolio` 已实现多图直传、自动清单资料、草稿、预览、发布、隐藏、归档、替换、排序和孤儿上传补偿；首图生产生命周期已闭环，下一步是其余图片的五语言资料、分类与地点审核 |
