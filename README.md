...
| 优先级 | 内容 | 说明 |
|--------|------|------|
| 1 · P0 | **巴厘岛剩余 POI 来源复核** | R1 主干、R2、R3、R4 与 R6 的免费路线稳定事实已核验；全库现为 59 个 POI：53 个 `verified`、3 个 `pending_review`、3 个 `needs_supplier_confirmation`。Thousand Islands Viewpoint、Mount Batur 徒步入口和 Batur 温泉须先确定唯一规范身份、入口或场所；供应商体验继续受真实运营资料门禁。验收：公开路线引用的稳定事实有官方/政府或场所一手来源，营业时间、票价、课程、仪式、火山状态、天气、水况、船班与交通继续在出发前实时复核 |
...
| 4 · P1 | **批量 Portfolio 治理** | 首图生产生命周期已闭环；首批 15 张同时关联区域、路线与已核验 POI 的 D8 图片已补齐中/英/日/韩/印五语言标题、说明和替代文本，并通过原图哈希匹配与 390/768/1440 响应式自动填充验收。剩余 37 张 D8 候选仍需按地点证据、路线关联和五语言资料分批审核；未确认素材不可发布，公开 API 与 UI 数量口径必须一致 |
