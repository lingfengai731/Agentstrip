...
| 图片治理 | 🟡 首图草稿已完成 | 108 张候选素材均已记录统一授权、哈希、Web 优化副本与发布 manifest；`Pura Tanah Lot.jpg` 已在生产完成对象存储直传、数据库草稿记录和预览，清单匹配会自动带入五语言资料；剩余门禁为发布→公开读取→隐藏回滚，未完成前不批量迁移 |
...
> Portfolio 生产环境已配置 `CLOUDINARY_CLOUD_NAME`、`CLOUDINARY_API_KEY`、`CLOUDINARY_API_SECRET` 与强管理员账号；管理入口仅对登录管理员显示。真实首图已完成“对象存储已就绪”→上传草稿，且草稿不会进入公开 API；仍需完成发布检查→隐藏回滚，未闭环前不批量迁移图片。
...
| 🟡 内容管理 | **Portfolio Content Manager** | `/admin/portfolio` 已实现多图直传、自动清单资料、草稿、预览、发布、隐藏、归档、替换和排序，并保留 37 张静态图片作为故障回退；首张授权图片已完成真实草稿上传，当前门禁是公开读取与隐藏回滚验收 |
