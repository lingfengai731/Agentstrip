...
| PayPal 自动支付 | 🟡 Sandbox 主路径与取消状态机已部署、待外部异常流与 Live 门禁 | 站长已用同一 Sandbox Personal 买家完成 USD 1.49 付款，并在刷新、重新进入、行程调整及账户入口中确认权益持续恢复，成功支付阶段已闭环。PR #39 已部署买家取消后的本地订单关闭与全新重试、退款复核队列可见性；87 项权限测试及生产 Chromium 取消态/全页面矩阵通过。拒绝、重复 webhook、退款已有本地状态机测试，但真实 Sandbox webhook/退款与 Live 资金仍是独立门禁，不能把 Sandbox 成功等同于真实收款 |
...
| 响应式与回归 | ✅ 生产与真实账号验收通过 | PR #34、#36、#37 已合并，Render 已提供 `ai-tool.js?v=p59`；生产 320/390/768/1440 的 Bali、套餐、搜索、账户入口、AI 深链与司机交接矩阵通过。站长随后用同一已付款 Sandbox 账号确认完整路线持续开放、调整入口与账户入口正常，无需再次付款 |

> 当前生产已显示 PayPal Sandbox 测试入口，并保留微信/支付宝二维码 + 管理员人工确认。成功付款与同账号权益恢复已经站长验收；Sandbox 仍只使用测试账户和测试资金。切换 Live 前必须用一次性测试订单完成取消、拒付、重复 webhook、退款与对账验收，并保留强管理员密码。
...
| 4 · P0 | **生产写入型 E2E** | 同一已购 Sandbox 账号的权益恢复、完整路线、调整入口和账户入口已经站长验收，成功支付阶段关闭。旅行豆扣减、调整扣次、管理员人工确认/无限权限及 PayPal 外部取消、拒绝、重复 webhook、退款仍需一次性测试数据和可回滚矩阵；不得对现有已购测试订单重复付款或退款 |
...
| 5 · P1 | **PayPal 自动支付生产闭环** | 六项 Render Sandbox 变量与 webhook 已配置；Sandbox Personal 成功付款及同账号权益恢复已完成。本地工程门禁现覆盖取消后全新重试、拒绝/待处理、重复 webhook、退款进入人工复核及管理员可见参考号；外部验收仍需一次性 Sandbox 订单完成取消/拒绝、真实 webhook、退款与对账，再单独决定 Live 凭据、Live webhook 和最小真实交易 |
...
| 13 · P1 | **微信小程序 v1 与发布门禁** | v1 已接入账号、验证码、AI、R1–R6、专业路线权益、历史、司机交接和微信官方内容安全核验；用户资料和文本完整分块、失败关闭。request 合法域名、隐私、UGC、Render 服务端凭据、PR #45 与生产端点已验收。下一门禁是真机 Preview；未上传、未提审、未发布 |
