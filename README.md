...
| 4 · P1 | **批量 Portfolio 治理** | 首图生产生命周期已闭环；当前 24 张 D8 图片已补齐中/英/日/韩/印五语言标题、说明和替代文本。`bali-12.jpg` 已由原图哈希和场所影像证据关联乌布圣猴森林；`Galungan.jpg` 以“与加隆安相关的 penjor”限定表述关联 R4；文件名为 `Nyepi.jpg` 的图片已按门牌与画面事实纠正为塞米亚克村社神庙的社区文化聚会，关联 G1/R6，但不绑定 POI，也不开放 AI/司机交接。剩余 28 张仍按地点证据、路线关联和五语言资料分批审核。未确认素材不可发布，公开 API 与 UI 数量口径必须一致 |
