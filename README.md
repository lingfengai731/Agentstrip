...
| 图片治理 | 🟡 首图验收门禁 | 108 张候选素材均已记录统一授权、哈希、Web 优化副本与发布 manifest；首张真实单图 E2E 候选为 `Pura Tanah Lot.jpg`，已补齐五语言元数据和重复 SHA 门禁；生产 Cloudinary 三项变量与管理员登录已配置，下一步是在管理页确认对象存储状态并完成草稿→发布→隐藏回滚闭环 |
...
> Portfolio 生产环境已配置 `CLOUDINARY_CLOUD_NAME`、`CLOUDINARY_API_KEY`、`CLOUDINARY_API_SECRET` 与强管理员账号；管理入口仅对登录管理员显示。真实首图仍需完成“对象存储已就绪”→上传草稿→发布检查→隐藏回滚的生产验收，未闭环前不批量迁移图片。
...
| 优先级 | 内容 | 说明 |
|--------|------|------|
| 🟡 内容核验 | **巴厘岛 POI 逐条来源复核** | 当前 50 个路线规划点均明确标记 `pending_review`，不能把规划锚点宣传为已核验营业信息；需按官方/可靠来源逐条补开放时间、预约和交通证据 |
| 🟡 内容管理 | **Portfolio Content Manager** | `/admin/portfolio` 第一阶段已实现多图直传、草稿、预览、发布、隐藏、归档、替换和排序，并保留 37 张静态图片作为故障回退；Cloudinary 与管理员环境已配置，当前门禁是首张授权图片的真实上传、公开读取和回滚验收 |
