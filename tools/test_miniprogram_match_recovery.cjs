...
async function run() {
  ...
  assert.equal(request.timeout,45000,'professional matching must allow slow mobile networks but remain bounded');
  ...
  for(const lang of ['zh','en','ja','ko','id']){
    ...
    vm.runInNewContext(source('miniprogram/pages/planner/planner.js'),{
      Page:p=>planner=p,getApp:()=>plannerApp,require:n=>n.includes('network-check')?require('../miniprogram/utils/network-check.js'):n==='./copy.js'?require('../miniprogram/pages/planner/copy.js'):{createProfessionalRoute:()=>new Promise(r=>finish=r)},
