...
> 2026-09-05 更新：PayPal China Live 审核已要求旅行/运输资质，原先“优先上线 PayPal”的判断不再成立。当前只保留 Sandbox，最新定案与下一步见 `PAYMENT_COMPLIANCE_ROUTE_2026-09-05.md`。不得虚报商户分类，也不得把数字路线付款与司机/交通服务款混在同一订单中。
...
- 绑定与主体一致的结算账户并完成审核。
- 先在 Sandbox 创建一个 `USD 1.49` 的专业路线测试订单，不使用真实银行卡扣款。`USD 1.49` 只用于打通流程，不代表已经确定生产汇率或最终实收币种。
...
- `USD 1.49` 仅适合作为 Sandbox 流程测试值；网站的 `¥9.9` 是产品首发价，不等于 PayPal 已确认可直接用 CNY 跨境结算；
