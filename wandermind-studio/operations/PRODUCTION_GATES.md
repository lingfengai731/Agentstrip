...
| Automatic payment | PayPal Orders v2 Sandbox, server-side amount checks and signed/idempotent webhook handling are implemented; the owner completed one Sandbox buyer payment and confirmed same-account entitlement restoration. Manual QR plus admin confirmation remains available. | Disposable Sandbox evidence for buyer cancel, provider decline, webhook redelivery, refund and reconciliation; explicit refund/entitlement policy; then separate Live credentials, Live webhook and minimal real-money authorization. | Independent production gate. Never refund the owner's existing accepted test order, and never present Sandbox or local mocks as Live settlement. |
