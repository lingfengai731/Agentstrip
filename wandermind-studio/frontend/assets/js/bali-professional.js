(function () {
  ...
  var PAYPAL_COPY = {
    en: { option:'Pay for digital route access with PayPal or card', sandbox:'Sandbox test · no real charge', local:'Or use a local QR payment', processing:'Verifying the payment securely…', done:'Payment verified. Your digital route is now open.', cancelled:'Checkout closed. No route access was granted; you can try again.', failed:'PayPal could not verify this payment. No route access was granted.' },
    zh: { option:'使用 PayPal 或银行卡购买数字路线访问权', sandbox:'沙盒测试 · 不会真实扣款', local:'或使用本地二维码付款', processing:'正在由服务器安全核验付款…', done:'付款已核验，数字路线已经开放。', cancelled:'已关闭付款，没有授予路线权益；你可以重新尝试。', failed:'PayPal 未能核验这笔付款，路线尚未解锁。' },
  ...
  var OFFLINE_COPY = {
    en: { qrTitle:'Mainland China QR payment', qrText:'WeChat Pay and Alipay are confirmed manually by the site owner.', qrPaid:'I paid by WeChat Pay or Alipay' },
    zh: { qrTitle:'中国大陆扫码付款', qrText:'微信和支付宝由站长人工核对到账。', qrPaid:'我已通过微信或支付宝付款' },
    ja: { qrTitle:'中国本土QR決済', qrText:'WeChat PayとAlipayはサイト運営者が入金を確認します。', qrPaid:'WeChat Pay / Alipayで支払い済み' },
    ko: { qrTitle:'중국 본토 QR 결제', qrText:'WeChat Pay와 Alipay 입금은 사이트 운영자가 직접 확인합니다.', qrPaid:'WeChat Pay / Alipay 결제 완료' },
    id: { qrTitle:'Pembayaran QR Tiongkok daratan', qrText:'WeChat Pay dan Alipay dikonfirmasi secara manual oleh pemilik situs.', qrPaid:'Saya sudah bayar lewat WeChat Pay / Alipay' }
  };

  var COPY = {
    en: {
      noProfile: 'Tell us the basics and we will match a route to your dates, pace and budget. You can also skip this and keep browsing the free public routes.',
      ...
      unlock: 'Unlock digital route · ¥9.9', points: 'Use 30 referral points', adjust: 'Adjust this trip', remaining: '{n} adjustments left', adjustExhausted: 'No adjustments left', adjustScope: 'Includes 3 parameter adjustments for this same trip: dates or days, pace, budget, interests, travel style and group size. It does not include human deep customization or new trip orders.', routeSwitchPending: '{route} is selected above. Your unlocked route will not be replaced by browsing; submit “Adjust this trip” to switch, using one adjustment.', driver: 'Send this route to a driver', dicky: 'Send to Dicky', gede: 'Send to Gede Nico', login: 'Sign in to unlock', public: 'Browse public R1–R6 routes', payTitle: 'Unlock digital route access', payText: 'Pay ¥9.9 for digital access to this route and 3 parameter adjustments for the same trip. Transport, driver fees and bookings are not included; local services are quoted and paid separately to the provider.', paid: 'I paid · submit for confirmation', orderSent: 'Payment confirmation request submitted. The route will open after confirmation.', pointsDone: 'Route unlocked with points.', loading: 'Matching your route…', error: 'We could not load the professional route. Please try again.', noData: 'Enter your dates, travellers and budget to start.', routeBasis: 'Built around your dates, group, budget, pace and interests. Opening hours and availability are checked again before travel.'
    },
    zh: {
      noProfile: '告诉我们日期、人数、预算和节奏，就能匹配更适合你的路线。暂时不填，也可以继续浏览免费公共路线。',
  ...
  // Customer-facing copy describes concrete choices; internal route and region IDs stay in data only.
  Object.assign(COPY.en, { noProfile:'Tell us your dates, group and budget to match a route. You can also keep browsing the free public routes.', pace:'Places per day', routeBasis:'Built around your dates, group, budget, places per day and interests. Opening hours and availability are checked again before travel.' });
  Object.assign(COPY.zh, { noProfile:'填写日期、人数和预算即可匹配路线；也可以先浏览免费公共路线。', pace:'每天安排几处', routeBasis:'根据日期、同行人数、预算、每天地点数和偏好组合。开放时间和可用性会在出发前再次确认。' });
  Object.assign(COPY.ja, { intro:'日程、人数、目的、予算を入力すると、約1分で実用的なバリ島ルートを提案します。', pace:'1日の訪問数', slow:'1日1〜2か所', balanced:'1日2〜3か所', active:'1日3〜4か所', hidden:'人の少ない場所', routeFamily:'確認済みの6つのバリ島ルートから提案', routeR1:'初めてのバリ島定番', routeR2R3:'スロー旅または南部海岸', routeR4:'文化とローカル体験', routeR5:'アウトドアと火山', routeR6:'景色と島の暮らし', reassurance:'同じ旅程を3回調整できます。', noProfile:'日程、人数、予算を入力するとルートを提案します。公開ルートはそのまま閲覧できます。', routeBasis:'日程、人数、予算、1日の訪問数、興味に合わせて構成します。営業時間と空き状況は出発前に再確認します。' });
  Object.assign(COPY.ko, { intro:'날짜, 인원, 관심사와 예산을 입력하면 약 1분 안에 실행 가능한 발리 경로를 추천합니다.', pace:'하루 방문 장소 수', slow:'하루 1–2곳', balanced:'하루 2–3곳', active:'하루 3–4곳', hidden:'덜 붐비는 장소', routeFamily:'검토된 발리 경로 6가지 중에서 추천', routeR1:'첫 발리 핵심 코스', routeR2R3:'느린 여행 또는 남부 해안', routeR4:'문화와 현지 체험', routeR5:'야외 활동과 화산', routeR6:'풍경과 섬 생활', reassurance:'같은 여행을 3회 조정할 수 있습니다.', noProfile:'날짜, 인원과 예산을 입력하면 경로를 추천합니다. 공개 경로는 계속 볼 수 있습니다.', routeBasis:'일정, 인원, 예산, 하루 방문 장소 수와 관심사에 맞춰 구성합니다. 운영 시간과 이용 가능 여부는 출발 전에 다시 확인합니다.' });
  Object.assign(COPY.id, { intro:'Isi tanggal, jumlah orang, prioritas, dan anggaran. Kami akan mencocokkan rute Bali yang praktis dalam sekitar satu menit.', pace:'Tempat per hari', slow:'1–2 tempat per hari', balanced:'2–3 tempat per hari', active:'3–4 tempat per hari', hidden:'Tempat yang lebih sepi', routeFamily:'Dicocokkan dari enam gaya rute Bali yang sudah ditinjau', routeR1:'Inti Bali untuk kunjungan pertama', routeR2R3:'Bali santai atau pesisir selatan', routeR4:'Budaya dan pengalaman lokal', routeR5:'Alam terbuka dan gunung berapi', routeR6:'Pemandangan dan kehidupan pulau', reassurance:'Rute yang sama dapat disesuaikan 3 kali.', noProfile:'Isi tanggal, jumlah orang, dan anggaran untuk mencocokkan rute. Rute publik tetap dapat dilihat.', routeBasis:'Disusun menurut tanggal, jumlah orang, anggaran, jumlah tempat per hari, dan minat. Jam buka serta ketersediaan diperiksa kembali sebelum berangkat.' });
  ...
  function formMarkup(profile) {
    ...
    return '<form class="bali-professional-form" id="bali-professional-form" novalidate>' + stepMarkup(1) +
      ...
      '<div class="bali-professional-field"><span class="bali-professional-field-label">' + esc(l.hotelArea) + '</span><span class="bali-professional-field-help">' + esc(l.hotelHint) + '</span><select class="bali-professional-select" name="hotel_area" aria-label="' + esc(l.hotelArea) + '">' + hotelOptionsMarkup(l, hotelArea) + '</select></div>' +
  ...
  function bindResultActions() {
    ...
    if (unlock) unlock.onclick = async function () { if (!isLoggedIn()) { redirectToLogin(); return; } await (state.paypal ? Promise.resolve(state.paypal) : loadPayPalConfig()); state.paymentOpen = true; renderResult(); };
