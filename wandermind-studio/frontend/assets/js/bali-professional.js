(function () {
  ...
  async function adjustRoute(profile) {
    ...
    try {
      var response = await fetch(API_BASE + '/api/bali/professional-route/' + encodeURIComponent(state.tripId) + '/adjust', { method:'POST', headers:Object.assign({ 'Content-Type':'application/json', 'X-Anon-Id':sessionId() }, authHeaders()), body:JSON.stringify({ trip_profile:profile, route_id:'', lang:currentLang() }) });
      var body = await response.json().catch(function () { return {}; });
