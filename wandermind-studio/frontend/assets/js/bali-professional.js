(function () {
  ...
  var COPY = {
    en: {
      ...
      formTitle: 'Your Bali trip', submit: 'Match my route', edit: 'Adjust trip information', cancel: 'Close editor',
      ...
      unlock: 'Unlock full route · ¥9.9', points: 'Use 30 referral points', adjust: 'Re-match route', remaining: '{n} adjustments left', driver: 'Send this route to a driver', dicky: 'Send to Dicky', gede: 'Send to Gede Nico', login: 'Sign in to unlock', public: 'Browse public R1–R6 routes', payTitle: 'Unlock this route', payText: 'Pay ¥9.9 by QR, then submit the payment for manual confirmation. The professional route stays separate from AI self-planning credits.', paid: 'I paid · submit for confirmation', orderSent: 'Payment confirmation request submitted. The route will open after confirmation.', pointsDone: 'Route unlocked with points.', loading: 'Matching your route…', error: 'We could not load the professional route. Please try again.', noData: 'Enter your dates, travellers and budget to start.', routeBasis: 'Structured from Bali geography, route families and POI modules. Access and availability still need confirmation.'
    },
    zh: {
      noProfile: '完成几项旅行信息后，我们可以从 Bali 的区域和路线体系中匹配更适合你的专业路线。你也可以跳过，继续浏览公共路线。',
      formTitle: '你的巴厘岛行程', submit: '匹配我的路线', edit: '调整旅行信息', cancel: '收起编辑',
      ...
      unlock: '解锁完整路线 · ¥9.9', points: '用 30 推荐积分兑换', adjust: '重新匹配路线', remaining: '还可调整 {n} 次', driver: '把完整路线发给司机', dicky: '发送给 Dicky', gede: '发送给 Gede Nico', login: '登录后解锁', public: '继续浏览公共 R1–R6 路线', payTitle: '解锁这条路线', payText: '使用二维码支付 ¥9.9，再提交到账确认。专业路线权益与 AI 自助规划额度分开记录。', paid: '我已付款 · 提交确认', orderSent: '到账确认申请已提交，管理员确认后路线会开放。', pointsDone: '已用积分解锁路线。', loading: '正在匹配你的路线…', error: '专业路线暂时无法加载，请稍后重试。', noData: '填写日期、人数和预算后开始匹配。', routeBasis: '基于 Bali 地理区域、路线家族和 POI 模块生成。开放时间和可用性仍需确认。'
    },
    ja: { noProfile:'旅行情報を入力すると、Bali の地域とルートから合うプロルートを提案します。公開ルートはそのまま見られます。', formTitle:'バリ旅行の情報', submit:'ルートを提案', edit:'旅行情報を調整', cancel:'編集を閉じる', audience:'旅行段階', first:'初めて', returning:'リピーター', people:'人数', start:'出発日', end:'帰着日', budget:'予算', style:'旅行スタイル', comfort:'快適', budgetStyle:'予算重視', luxury:'プレミアム', pace:'ペース', balanced:'バランス', slow:'ゆっくり', goals:'優先事項', local:'文化', photo:'風景・写真', easy:'計画を減らす', value:'予算管理', route:'提案されたプロルート', preview:'無料プレビュー', unlocked:'完全版を解放済み', reason:'おすすめの理由', openDays:'{n}日を表示', lockedDays:'{n}日をロック', day:'{n}日目', locked:'詳細はロック中', lockedNote:'解放すると場所の順序、体験モジュール、実行メモを確認できます。', unlock:'完全版を解放 · ¥9.9', points:'紹介ポイント30で交換', adjust:'ルートを再調整', remaining:'残り{n}回', driver:'完全なルートをドライバーへ', dicky:'Dickyへ送る', gede:'Gede Nicoへ送る', login:'ログインして解放', public:'公開R1–R6を見る', payTitle:'このルートを解放', payText:'QRで¥9.9を支払い、確認を申請します。AI自分計画の枠とは別管理です。', paid:'支払済み・確認を申請', orderSent:'確認申請を受け付けました。確認後に開放されます。', pointsDone:'ポイントで解放しました。', loading:'ルートを提案中…', error:'プロルートを読み込めませんでした。', noData:'日付、人数、予算を入力してください。', routeBasis:'Baliの地理、ルート、POIモジュールから構成しています。時間と空き状況は要確認です。' },
    ko: { noProfile:'여행 정보를 입력하면 Bali 지역과 경로에서 맞춤 전문 루트를 추천합니다. 공개 경로는 계속 볼 수 있습니다.', formTitle:'발리 여행 정보', submit:'경로 매칭', edit:'여행 정보 조정', cancel:'편집 닫기', audience:'여행 단계', first:'첫 방문', returning:'재방문', people:'인원', start:'출발일', end:'귀국일', budget:'예산', style:'여행 스타일', comfort:'편안함', budgetStyle:'예산 우선', luxury:'프리미엄', pace:'속도', balanced:'균형', slow:'느긋하게', goals:'우선순위', local:'현지 문화', photo:'풍경·사진', easy:'계획 줄이기', value:'예산 관리', route:'매칭된 전문 루트', preview:'무료 미리보기', unlocked:'전체 루트 잠금 해제', reason:'추천 이유', openDays:'{n}일 공개', lockedDays:'{n}일 잠금', day:'{n}일차', locked:'상세 잠금', lockedNote:'잠금 해제 후 장소 순서, 체험 모듈과 실행 메모를 확인할 수 있습니다.', unlock:'전체 루트 잠금 해제 · ¥9.9', points:'추천 포인트 30점 사용', adjust:'루트 다시 매칭', remaining:'{n}회 남음', driver:'전체 루트를 기사에게 보내기', dicky:'Dicky에게 보내기', gede:'Gede Nico에게 보내기', login:'로그인 후 잠금 해제', public:'공개 R1–R6 보기', payTitle:'이 루트 잠금 해제', payText:'QR로 ¥9.9를 결제하고 확인을 요청합니다. AI 직접 계획 한도와 별도입니다.', paid:'결제 완료 · 확인 요청', orderSent:'확인 요청을 보냈습니다. 확인 후 루트가 열립니다.', pointsDone:'포인트로 잠금 해제했습니다.', loading:'루트를 매칭하는 중…', error:'전문 루트를 불러오지 못했습니다.', noData:'날짜, 인원과 예산을 입력하세요.', routeBasis:'Bali 지리, 경로 가족과 POI 모듈을 바탕으로 구성합니다. 운영 시간과 이용 가능 여부는 확인이 필요합니다.' },
    id: { noProfile:'Isi detail perjalanan untuk mencocokkan rute profesional dari wilayah dan rute Bali. Anda tetap dapat melihat rute publik.', formTitle:'Perjalanan Bali Anda', submit:'Cocokkan rute saya', edit:'Ubah detail perjalanan', cancel:'Tutup editor', audience:'Tahap perjalanan', first:'Pertama kali', returning:'Pernah datang', people:'Jumlah orang', start:'Tanggal berangkat', end:'Tanggal pulang', budget:'Anggaran', style:'Gaya perjalanan', comfort:'Nyaman', budgetStyle:'Hemat', luxury:'Premium', pace:'Tempo', balanced:'Seimbang', slow:'Santai', goals:'Prioritas', local:'Budaya lokal', photo:'Pemandangan & foto', easy:'Lebih sedikit rencana', value:'Kendali anggaran', route:'Rute profesional yang cocok', preview:'Pratinjau gratis', unlocked:'Rute lengkap terbuka', reason:'Alasan rekomendasi', openDays:'{n} hari terbuka', lockedDays:'{n} hari terkunci', day:'Hari {n}', locked:'Detail terkunci', lockedNote:'Buka kunci untuk melihat urutan tempat, modul pengalaman dan catatan pelaksanaan.', unlock:'Buka rute lengkap · ¥9.9', points:'Gunakan 30 poin referral', adjust:'Cocokkan ulang', remaining:'tersisa {n} penyesuaian', driver:'Kirim rute lengkap ke driver', dicky:'Kirim ke Dicky', gede:'Kirim ke Gede Nico', login:'Masuk untuk membuka', public:'Lihat rute publik R1–R6', payTitle:'Buka rute ini', payText:'Bayar ¥9.9 melalui QR lalu kirim untuk konfirmasi manual. Hak rute profesional terpisah dari kredit AI.', paid:'Saya sudah bayar · kirim konfirmasi', orderSent:'Permintaan konfirmasi dikirim. Rute terbuka setelah dikonfirmasi.', pointsDone:'Rute dibuka dengan poin.', loading:'Mencocokkan rute…', error:'Rute profesional tidak dapat dimuat.', noData:'Isi tanggal, jumlah orang dan anggaran.', routeBasis:'Disusun dari geografi Bali, keluarga rute dan modul POI. Jam buka dan ketersediaan tetap perlu dikonfirmasi.' }
  };
  ...
  function makeProfile(form) {
    ...
    return {
      ...
      currency: 'CNY', budget_range: Number(form.querySelector('[name="budget"]').value || 0),
  ...
  function formMarkup(profile) {
    ...
    return '<form class="bali-professional-form" id="bali-professional-form">' +
      ...
      '<div class="bali-professional-form-row"><label>' + esc(l.pace) + '<select name="pace"><option value="balanced" ' + (p.pace !== 'slow' ? 'selected' : '') + '>' + esc(l.balanced) + '</option><option value="slow" ' + (p.pace === 'slow' ? 'selected' : '') + '>' + esc(l.slow) + '</option></select></label><label>' + esc(l.goals) + '<span><input type="checkbox" name="goal" value="local" ' + (goals.indexOf('local') >= 0 ? 'checked' : '') + '> ' + esc(l.local) + ' · <input type="checkbox" name="goal" value="photo" ' + (goals.indexOf('photo') >= 0 ? 'checked' : '') + '> ' + esc(l.photo) + '</span></label></div>' +
      '<div class="bali-professional-actions"><button class="bali-btn bali-btn-primary" type="submit"><span class="fa fa-magic"></span> ' + esc(l.submit) + '</button><a class="bali-btn bali-route-secondary" href="#route-families">' + esc(l.public) + '</a></div>' +
  ...
  function bindForm(form) {
    ...
    form.addEventListener('submit', function (event) {
      ...
      saveProfile(profile);
      state.editing = false;
      loadRoute(profile, state.routeId);
    });
  }
  ...
  function renderResult() {
    ...
    var actionHtml = unlocked ? '<button class="bali-btn bali-btn-primary" type="button" id="bali-professional-edit"><span class="fa fa-sliders"></span> ' + esc(l.adjust) + '</button><span class="bali-professional-badge">' + esc(fill(l.remaining, { n:data.professional_adjustments_remaining })) + '</span>' : '<button class="bali-btn bali-btn-primary" type="button" id="bali-professional-unlock"><span class="fa fa-lock"></span> ' + esc(l.unlock) + '</button><button class="bali-btn bali-route-secondary" type="button" id="bali-professional-points"><span class="fa fa-gift"></span> ' + esc(l.points) + '</button>';
  ...
  async function loadRoute(profile, routeId) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.detail || T().error);
      ...
      saveProfile(profile); renderResult();
