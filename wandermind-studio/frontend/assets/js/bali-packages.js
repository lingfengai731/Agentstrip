(function () {
  ...
  function copy() {
    return ({
      zh:{nav:'套餐',eyebrow:'巴厘岛模块 · 可编辑套餐',title:'从一日体验开始，组合成能落地的巴厘岛套餐',subtitle:'按天数查看套餐骨架，再由司机核对交通、供应商、安全、天气与最终价格。每个模块都可以替换，它不是即时预订。',days:'天数',all:'全部',one:'1 日',two:'2 日',select:'选择这个套餐',selected:'已选择',checks:'出发前确认',send:'把套餐发给司机确认',note:'套餐是可编辑的路线起点，不是即时预订。供应商、天气、保险与最终价格会在司机回复后确认。',empty:'没有符合筛选的套餐。'},
      en:{nav:'Packs',eyebrow:'Bali modules · editable packages',title:'Start with a one-day experience, then build a workable Bali package',subtitle:'Browse by days, then let a driver check transport, suppliers, safety, weather and final price. Every module can be swapped; this is not instant booking.',days:'Days',all:'All',one:'1 day',two:'2 days',select:'Choose this package',selected:'Selected',checks:'Confirm before travel',send:'Send package to a driver',note:'Packages are editable route starts, not instant bookings. Suppliers, weather, insurance and final price are confirmed after the driver replies.',empty:'No package matches these filters.'},
      ja:{nav:'パック',eyebrow:'バリのモジュール · 編集可能',title:'1日体験から、実行できるバリ旅へ',subtitle:'日数で選び、交通・事業者・安全・天候・最終価格をドライバーが確認します。各モジュールは入れ替え可能で、即時予約ではありません。',days:'日数',all:'すべて',one:'1日',two:'2日',select:'このパッケージを選ぶ',selected:'選択済み',checks:'出発前に確認',send:'ドライバーに確認を送る',note:'編集できる旅程の出発点であり、即時予約ではありません。最終条件はドライバー返信後に確認します。',empty:'条件に合うパッケージがありません。'},
      ko:{nav:'패키지',eyebrow:'발리 모듈 · 편집 가능',title:'하루 체험에서 실행 가능한 발리 패키지로',subtitle:'일수로 살펴본 뒤 교통·업체·안전·날씨·최종 가격을 기사가 확인합니다. 각 모듈은 교체할 수 있으며 즉시 예약이 아닙니다.',days:'일수',all:'전체',one:'1일',two:'2일',select:'이 패키지 선택',selected:'선택됨',checks:'출발 전 확인',send:'기사에게 패키지 확인 요청',note:'편집 가능한 일정 시작점이며 즉시 예약이 아닙니다. 최종 조건은 기사 답장 후 확인합니다.',empty:'필터와 일치하는 패키지가 없습니다.'},
      id:{nav:'Paket',eyebrow:'Modul Bali · dapat diedit',title:'Mulai dari pengalaman sehari, lalu susun paket Bali yang dapat dijalankan',subtitle:'Lihat berdasarkan jumlah hari, lalu pengemudi memeriksa transportasi, pemasok, keselamatan, cuaca dan harga akhir. Setiap modul dapat diganti; ini bukan pemesanan instan.',days:'Hari',all:'Semua',one:'1 hari',two:'2 hari',select:'Pilih paket ini',selected:'Dipilih',checks:'Konfirmasi sebelum berangkat',send:'Kirim paket ke pengemudi',note:'Paket adalah titik awal rute yang dapat diedit, bukan pemesanan instan. Detail akhir dikonfirmasi setelah pengemudi membalas.',empty:'Tidak ada paket yang sesuai.'}
    })[language()] || null;
  }
  ...
  function render() {
    ...
    root.innerHTML = renderFilters() + (list.length ? '<div class="bali-package-grid">'+list.map(function(item){
      ...
      return '<article class="bali-package-card'+(selected?' active':'')+'"><div class="bali-package-card-top"><span>'+esc((item.route_id || 'Bali')+' · '+durationLabel(item.duration_days, c))+'</span><span class="bali-package-status">'+esc(c.checks)+'</span></div><h3>'+esc(text(item.title))+'</h3><p>'+esc(text(item.summary))+'</p><button type="button" data-package-select="'+esc(item.id)+'" aria-pressed="'+String(selected)+'">'+esc(selected?c.selected:c.select)+'</button></article>';
