(function () {
  ...
  fetch('assets/data/bali-experience-packages.json?v=20260911p1').then(function(response){if(!response.ok)throw new Error('HTTP '+response.status);return response.json();}).then(function(data){packages=data.packages||[];transportPolicies=data.transport_policies||{};var requested=new URLSearchParams(window.location.search).get('package');if(packages.some(function(item){return item.id===requested;}))selectedId=requested;render();}).catch(function(){root.innerHTML='<p class="bali-package-empty">Package library is temporarily unavailable.</p>';});
