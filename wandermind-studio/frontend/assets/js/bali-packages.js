(function () {
  ...
  function renderSchedule(item, c) {
    var schedule = (transportPolicies.nusa_penida || {}).schedule;
    if (!schedule || item.supplier_candidate !== 'Axestone Fast Cruise') return '';
    return '<details class="bali-package-schedule"><summary>'+esc(c.schedule)+'</summary><p><strong>'+esc(c.outbound)+'</strong><br>'+esc((schedule.sanur_to_penida || []).join(' · '))+' '+esc(schedule.timezone || 'WITA')+'</p><p><strong>'+esc(c.inbound)+'</strong><br>'+esc((schedule.penida_to_sanur || []).join(' · '))+' '+esc(schedule.timezone || 'WITA')+'</p><small>'+esc(c.scheduleNote)+'</small><a href="'+esc(schedule.source_url)+'" target="_blank" rel="noopener">Axestone</a></details>';
  }
  ...
  fetch('assets/data/bali-experience-packages.json?v=20260901p5').then(function(response){if(!response.ok)throw new Error('HTTP '+response.status);return response.json();}).then(function(data){packages=data.packages||[];transportPolicies=data.transport_policies||{};var requested=new URLSearchParams(window.location.search).get('package');if(packages.some(function(item){return item.id===requested;}))selectedId=requested;render();}).catch(function(){root.innerHTML='<p class="bali-package-empty">Package library is temporarily unavailable.</p>';});
